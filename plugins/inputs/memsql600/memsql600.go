...
type memsql600 struct {
	DbUser     string   // memsql600 username
	DbPassword string   // memsql600 password
	DbHost     []string // memsql600 host
	Dbport     string   // memsql600 port
	DbName     string   // memsql600 database name
	Prot       string   // memsql600 connection protocol
	Files      string
	Interval   int
...
func (m *memsql600) SampleConfig() string {
	return sampleConfig
}

func (m *memsql600) Description() string {
	return "Query Stats from one or many memsql600 clusters"
}
...
func (m *memsql600) Gather(acc telegraf.Accumulator) error {
	if len(m.DbHost) == 0 {
...
func (m *memsql600) gatherServer(addr string, acc telegraf.Accumulator) error {
...
func (m *memsql600) readLines(path string) ([]string, error) {
...
func (m *memsql600) returnValue(pval *interface{}) interface{} {
	switch v := (*pval).(type) {
	case nil:
		return "NULL"
	case bool:
...
func (m *memsql600) runQueries(client *sql.DB, queryLines []string) []interface{} {
...
func init() {
	inputs.Add("memsql600", func() telegraf.Input {
		return &memsql600{
